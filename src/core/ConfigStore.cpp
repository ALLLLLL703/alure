...
QByteArray resource(const char *name) {
  ...
  if (!file.open(QIODevice::ReadOnly))
    throw std::runtime_error("Cannot open built-in defaults");
  return file.readAll();
}
QVariant convert(const toml::node &node) {
  if (const auto *table = node.as_table()) {
    ...
    for (const auto &[key, value] : *table)
      result.insert(QString::fromUtf8(key.str().data(), key.str().size()),
                    convert(value));
    return result;
  }
  ...
  if (auto v = node.value<std::string>(); node.is_string())
    return QString::fromStdString(*v);
  if (auto v = node.value<int64_t>(); node.is_integer())
    return QVariant::fromValue<qlonglong>(*v);
  if (auto v = node.value<double>(); node.is_floating_point())
    return *v;
  if (auto v = node.value<bool>(); node.is_boolean())
    return *v;
  // Unknown datetime values need not be interpreted; the original TOML remains
  // intact.
  return QVariant();
}
...
QVariantMap table(const QVariant &v, const QString &key) {
  if (v.metaType().id() != QMetaType::QVariantMap)
    invalid(key, "expected table");
  return v.toMap();
}
QVariantMap merge(QVariantMap base, const QVariantMap &user,
                  const QString &path = {}) {
  for (auto it = user.begin(); it != user.end(); ++it) {
    ...
    if (expected == QMetaType::QVariantMap) {
      base[it.key()] = merge(base.value(it.key()).toMap(),
                             table(it.value(), key), key + '.');
    } else {
      if (expected != actual &&
          !(expected == QMetaType::Double && actual == QMetaType::LongLong))
...
void range(const QVariantMap &m, const QString &key, double low, double high,
           const QString &path) {
  const double value = m.value(key).toDouble();
  if (!std::isfinite(value) || value < low || value > high)
    invalid(path + key,
            QString("must be between %1 and %2").arg(low).arg(high));
}
void choice(const QVariantMap &m, const QString &key, const QStringList &values,
            const QString &path) {
  if (!values.contains(m.value(key).toString()))
    invalid(path + key, "expected one of " + values.join(", "));
}
void nonempty(const QVariantMap &m, const QString &key, const QString &path) {
  if (m.value(key).toString().trimmed().isEmpty())
    invalid(path + key, "must not be empty");
}
bool readDisk(const QString &path, QByteArray &data, bool &exists,
              QString &error) {
...
QByteArray ConfigStore::defaultSource() {
  initDefaults();
  return resource(":/config/default.toml");
}
QStringList ConfigStore::themeNames() const {
  initDefaults();
  return convert(toml::parse(resource(":/config/themes.toml").toStdString()))
      .toMap()
      .keys();
}
bool ConfigStore::parse(const QByteArray &text, QVariantMap &model,
                        QString &error) {
  try {
    initDefaults();
    if (text.size() > maxConfigSize)
      invalid("config", "exceeds 1 MiB");
    const auto defaults =
        convert(toml::parse(defaultSource().toStdString())).toMap();
    ...
    const auto themes =
        convert(toml::parse(resource(":/config/themes.toml").toStdString()))
            .toMap();
    choice(theme, "name", themes.keys(), "theme.");
    auto palette = themes.value(theme.value("name").toString()).toMap();
    if (theme.contains("palette"))
      palette = merge(palette, table(theme.value("palette"), "theme.palette"),
                      "theme.palette.");
    for (auto it = palette.begin(); it != palette.end(); ++it)
      if (!QColor::isValidColorName(it.value().toString()))
        invalid("theme.palette." + it.key(), "invalid Qt color");
    ...
    choice(ui, "popup_direction", {"inward", "top", "bottom", "left", "right"},
           "ui.");
    ...
    if (!shortcut.isEmpty() &&
        (sequence.isEmpty() ||
         sequence.toString(QKeySequence::PortableText) != shortcut))
      invalid("settings.close_shortcut",
              "expected a canonical Qt portable shortcut, e.g. Ctrl+W, or "
              "empty to disable");
    const auto foundation = result.value("foundation").toMap();
    static const QRegularExpression iconName(
        QStringLiteral("^[A-Za-z0-9_.-]+$"));
    if (!iconName.match(foundation.value("icon").toString()).hasMatch())
      invalid("foundation.icon", "expected a nonempty icon name (letters, "
                                 "digits, _, . or -), not a path");
    if (!iconName.match(ui.value("settings_icon").toString()).hasMatch())
      invalid("ui.settings_icon", "expected icon name");
    auto modules = result.value("modules").toMap();
    auto moduleDefaults =
        defaults.value("modules").toMap().value("media").toMap();
    ...
    for (auto it = modules.begin(); it != modules.end(); ++it) {
      ...
      if (style.value("min_width").toInt() > style.value("max_width").toInt())
        invalid(stylePath + "min_width", "exceeds max_width");
      if (!iconName.match(style.value("icon").toString()).hasMatch())
        invalid(stylePath + "icon", "expected icon name, not a path");
      for (const auto &key : {"foreground", "background"})
        if (!style.value(key).toString().isEmpty() &&
            !QColor::isValidColorName(style.value(key).toString()))
          invalid(stylePath + key, "invalid Qt color");
      ...
      const auto knownBehavior = defaults.value("modules")
                                     .toMap()
                                     .value(it.key())
                                     .toMap()
                                     .value("behavior")
      ...
      for (auto option = behavior.begin(); option != behavior.end(); ++option) {
        if (option.key() != "command" &&
            !(option.key().endsWith("_command") &&
              knownBehavior.contains(option.key())))
          continue;
        if (option.value().metaType().id() != QMetaType::QVariantList)
          invalid(path + option.key(), "expected argv array");
        const auto argv = option.value().toList();
        if (argv.size() > 128)
          invalid(path + option.key(), "maximum 128 arguments");
        for (const auto &arg : argv)
          if (arg.metaType().id() != QMetaType::QString ||
              arg.toString().contains(QChar::Null))
            invalid(path + option.key(), "expected strings without NUL");
        if (!argv.isEmpty() && argv.first().toString().trimmed().isEmpty())
          invalid(path + option.key(), "executable must not be empty");
      }
      if (it.key() == "calendar")
        range(behavior, "first_day_of_week", 0, 1, path);
      if (it.key() == "tray") {
      ...
      if (it.key() == "tray_launcher" || it.key() == "media_launcher") {
        ...
        for (const auto &key : {"next_shortcut", "previous_shortcut", "tab_shortcut", "reverse_tab_shortcut", "activate_shortcut", "refresh_shortcut", "back_shortcut", "close_shortcut"}) {
          ...
          if (!shortcut.isEmpty() && (sequence.isEmpty() || sequence.toString(QKeySequence::PortableText) != shortcut))
            invalid(path + key, "expected canonical Qt shortcut or empty");
        }
      }
      if (it.key() == "clipboard") {
        ...
        for (const auto &key :
             {"copy_shortcut", "next_shortcut", "previous_shortcut"}) {
          const auto shortcut = behavior.value(key).toString();
          const auto sequence =
              QKeySequence::fromString(shortcut, QKeySequence::PortableText);
          if (!shortcut.isEmpty() &&
        ...
        if (database.contains(QChar::Null) ||
            (!database.isEmpty() && !QDir::isAbsolutePath(database)))
          invalid(path + "database_path",
                  "expected empty or absolute path without NUL");
        if (behavior.value("output").toString().isEmpty() ||
            behavior.value("output").toString().contains(QChar::Null))
          invalid(path + "output", "expected primary or an output name");
        for (const auto &key : {"cliphist_command", "copy_command"})
          if (behavior.value(key).toList().isEmpty() ||
      ...
      if (it.key() == "media" || it.key() == "media_launcher") {
        ...
        if (!preferred.isEmpty() &&
            !QRegularExpression("^org\\.mpris\\.MediaPlayer2\\.[A-Za-z0-9_-]+(?"
                 ...
                 .match(preferred)
                 .hasMatch())
          invalid(path + "preferred_player",
                  "expected empty or a full MPRIS service name/prefix");
      }
      ...
      if (it.key() == "notifications") {
        ...
        if (socket.contains(QChar::Null) ||
            (!socket.isEmpty() && !QDir::isAbsolutePath(socket)))
          invalid(path + "niri_socket",
                  "expected empty or absolute socket path");
        ...
        if (behavior.value("default_expire_ms").toInt() >
            behavior.value("max_expire_ms").toInt())
          invalid(path + "default_expire_ms", "exceeds max_expire_ms");
      }
      if (it.key() == "taskbar") {
      ...
      if (it.key() == "workspaces" || it.key() == "taskbar") {
        ...
        if (socket.contains(QChar::Null) ||
            (!socket.isEmpty() && !QDir::isAbsolutePath(socket)))
          invalid(path + "socket_path",
                  "expected empty or absolute socket path without NUL");
      }
      if (it.key() == "battery") {
        nonempty(behavior, "sysfs_path", path);
        if (behavior.value("sysfs_path").toString().contains(QChar::Null) ||
            !QDir::isAbsolutePath(behavior.value("sysfs_path").toString()))
          invalid(path + "sysfs_path", "expected absolute directory path");
      }
      it.value() = module;
    }
    result["modules"] = modules;
    QVariantList panels;
    const auto panelDefaults =
        defaults.value("panels").toList().first().toMap();
    QSet<QString> ids;
    const auto configuredPanels = result.value("panels").toList();
    if (configuredPanels.size() > 128)
      invalid("panels", "maximum 128 definitions");
    for (const auto &entry : configuredPanels) {
      ...
      if (ids.contains(id))
        invalid("panels.id", "duplicate " + id);
      ids.insert(id);
      nonempty(panel, "output", "panels[].");
      choice(panel, "edge", {"top", "bottom", "left", "right"}, "panels[].");
      choice(panel, "layer", {"background", "bottom", "top", "overlay"},
             "panels[].");
      ...
      for (const auto &key : {"top", "right", "bottom", "left"})
        range(margins, key, 0, 4096, "panels[].margins.");
      ...
      for (const auto *key :
        ...
        if (entries.size() > 128)
          invalid(path, "maximum 128 entries");
        QSet<QString> used;
        for (const auto &name : entries) {
          const auto token = name.toString();
          if (name.metaType().id() != QMetaType::QString ||
              (!modules.contains(token) && token != "@spacer" &&
               token != "@stretch" && token != "@settings"))
            invalid(path, "unknown module " + token);
          if (token == "@spacer" || token == "@stretch")
            continue;
          if (used.contains(token))
            invalid(path, "duplicate module " + token);
          used.insert(token);
          if (QString::fromLatin1(key) != "modules" &&
              panel.value("layout") == "three-zone") {
            if (zonedModules.contains(token))
              invalid(path, "module already placed in another zone: " + token);
    ...
    out << e;
    error = QString::fromStdString(out.str());
...
ConfigStore::ConfigStore(QString path, QObject *parent)
  ...
  const auto schedule = [this] {
    ...
    if (runtime.value("watch").toBool())
      m_reloadTimer.start(runtime.value("reload_delay_ms").toInt());
  };
...
bool ConfigStore::reload() {
  ...
  bool existed;
  QString error;
  if (!readDisk(m_path, bytes, existed, error)) {
    setDiagnostic(error);
    return false;
  }
  QVariantMap model;
...
bool ConfigStore::saveText(const QString &text) {
  ...
  QVariantMap model;
  QString error;
  if (!parse(bytes, model, error)) {
    setDiagnostic(error);
    return false;
  }
  if (!m_hasSnapshot) {
    setDiagnostic("Reload the configuration before saving");
    return false;
  }
  ...
  if (!QDir().mkpath(directory)) {
    setDiagnostic("Cannot create configuration directory");
    return false;
  }
  QLockFile lock(m_path + ".lock");
  if (!lock.tryLock(0)) {
    setDiagnostic("Configuration is locked by another writer");
    return false;
  }
  QByteArray current;
  bool existed;
  if (!readDisk(m_path, current, existed, error)) {
    setDiagnostic(error);
    return false;
  }
  if (existed != m_diskExisted || current != m_diskSnapshot) {
    setDiagnostic("Configuration changed externally; copy your edits, then "
                  "Reload before saving");
    return false;
  }
  // Do not replace a symlink with a regular file or follow it into another
  // config.
  if (QFileInfo(m_path).isSymLink()) {
    setDiagnostic("Saving through a symlink is not supported");
    return false;
  }
  QSaveFile file(m_path);
  file.setDirectWriteFallback(false);
  if (!file.open(QIODevice::WriteOnly) || file.write(bytes) != bytes.size()) {
    setDiagnostic(file.errorString());
    return false;
  }
  if (!readDisk(m_path, current, existed, error)) {
    setDiagnostic(error);
  ...
  if (existed != m_diskExisted || current != m_diskSnapshot ||
      QFileInfo(m_path).isSymLink()) {
    setDiagnostic(
        "Configuration changed while preparing save; Reload before saving");
    return false;
  }
  if (!file.commit()) {
    setDiagnostic(file.errorString());
...
void ConfigStore::startWatching() {
  m_watching = true;
  updateWatch();
}
void ConfigStore::updateWatch() {
  if (!m_watching)
    return;
  const auto oldFiles = m_watcher.files();
