...
function describe(path, value) {
    ...
    if (path.indexOf(".visibility.") >= 0) {
        if (key === "mode") options = ["always", "dodge-windows", "auto-hide"]
        if (key === "unknown_geometry") options = ["hide", "show"]
    }
    ...
    if (path.indexOf(".visibility.") >= 0) {
        if (key === "show_delay_ms" || key === "hide_delay_ms") range = [0,10000]
        if (key === "edge_trigger_px") range = [1,16]
    }
    ...
    if (path.startsWith("modules.taskbar.")) {
        ...
        help.interval_ms = "Reconnect retry interval; live windows and panel dodge geometry share a Niri event stream. Dodge keeps this stream enabled even when the taskbar UI is disabled."
        help.socket_path = "Empty uses NIRI_SOCKET. Shared by taskbar and panel dodge detection, independently of the workspace module."
    ...
    const group = path.indexOf(".visibility.") >= 0 ? "Panel visibility" : path.startsWith("modules.brightness.behavior.screen.") ? "Display backlight" : path.startsWith("modules.brightness.behavior.keyboard.") ? "Keyboard backlight" : path.indexOf("theme.palette.") === 0 ? "Theme colors" : path.indexOf("ui.osd.") === 0 ? "Volume / brightness OSD" : path.indexOf("ui.toast.") === 0 ? "Notification banners" : path.indexOf(".margins.") >= 0 ? "Panel margins" : ""
...
function panelGapHelp(panel) {
    if (!panel) return "Select a panel to configure its own window spacing."
    const scope = "Panel " + panel.id + " · " + panel.edge + " · " + panel.output + ": "
    const mode = (panel.visibility || {}).mode || "always"
    if (mode !== "always") return scope + mode + " reserves no space. Additional window gap and reserved screen space are ignored here; select the always-visible panel next to the unwanted gap."
    if (panel.exclusive_zone === 0) return scope + "reserved screen space is 0, so additional window gap has no effect."
    return scope + (panel.exclusive_zone === -1 ? "automatic thickness reservation" : "explicit reservation " + panel.exclusive_zone + " px")
        + ", adjusted by " + panel.window_gap + " px. This adjustment is not the final visible gap: compositor gaps/struts are separate. Negative values compensate them; screen-edge margins position the bar. Save & apply updates watching panels."
}
