...
| Volume | state: `percent`, `muted`, `backend`, `canSetVolume`, `canMute`; PulseAudio also reports sink identity and server name. `setVolume({percent})`, `adjustVolume({delta})`, `toggleMute({})`; `adjusting` property tracks pending writes/readback. | Async wpctl or pactl JSON; auto detects readable output and remembers the working backend. Coalesced setters/wheel input can queue while busy, configurable maximum, no mutation fallback. Default sink only; no microphone or stream mixer. See [audio backends](audio.md). |
