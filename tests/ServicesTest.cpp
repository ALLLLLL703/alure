...
private slots:
    ...
    void mediaAndBluetooth() {
        ...
        const auto mediaRow = media.items().first().toMap();
        QCOMPARE(mediaRow.value("title").toString(), "Fixture Song");
        QCOMPARE(mediaRow.value("artist").toStringList(), QStringList{"Fixture Artist"});
        const QVariantMap target{{"service", "org.mpris.MediaPlayer2.alureFixture"}, {"owner", mediaRow.value("owner")}};
        ...
        QVERIFY(media.action("setPosition", {{"service", target.value("service")}, {"owner", target.value("owner")}, {"trackId", "/fixture/track"}, {"positionUs", 9000000LL}}));
        QTRY_COMPARE(player.position, 9000000LL); QTRY_VERIFY(!media.busy());
        QVERIFY(!media.action("setPosition", {{"service", target.value("service")}, {"owner", target.value("owner")}, {"trackId", "/old/track"}, {"positionUs", 9000000LL}}));
        QVERIFY(!media.action("setPosition", {{"service", target.value("service")}, {"owner", target.value("owner")}, {"trackId", "/fixture/track"}, {"positionUs", "bad"}}));
        QVERIFY(!media.action("setPosition", {{"service", target.value("service")}, {"owner", target.value("owner")}, {"trackId", "/fixture/track"}, {"positionUs", 271000000LL}}));
        QVERIFY(media.action("setShuffle", {{"service", target.value("service")}, {"owner", target.value("owner")}, {"shuffle", true}})); QTRY_VERIFY(player.shuffled); QTRY_VERIFY(!media.busy());
        QVERIFY(media.action("setLoopStatus", {{"service", target.value("service")}, {"owner", target.value("owner")}, {"loopStatus", "Track"}})); QTRY_COMPARE(player.loop, "Track"); QTRY_VERIFY(!media.busy());
        QVERIFY(!media.action("setLoopStatus", {{"service", target.value("service")}, {"owner", target.value("owner")}, {"loopStatus", "Random"}}));
